...
static void run_demo_riscv() {
    ...
    cpu.run(35); // exact: 3 setup + 10x3 loop + bge-exit + nop (no overshoot)
...
// Full ISA coverage: executes all 37 RV32I instructions with hand-encoded
// machine code (vectors generated by host_test/gen_isa_test.py). Each result
// is stored to a table at 0x1200 and verified from C++ via read_mem_word().
static void run_demo_riscv_full() {
    ESP_LOGI(TAG, "=== Demo: RISC-V full ISA coverage (37 instructions) ===");
    RiscvCpu cpu;
    cpu.reset();
    cpu.load_program(0, ISA_PROGRAM, sizeof(ISA_PROGRAM) / sizeof(ISA_PROGRAM[0]));
    cpu.run(ISA_STEPS);
    int pass = 0;
    for (int i = 0; i < ISA_CHECKS; ++i) {
        uint32_t got = cpu.read_mem_word(0x1200 + i * 4);
        if (got == ISA_EXPECTED[i]) { ++pass; }
        else {
            ESP_LOGE(TAG, "[FAIL] %s: got 0x%08X expected 0x%08X",
                     ISA_NAMES[i], got, ISA_EXPECTED[i]);
        }
    }
    ESP_LOGI(TAG, "RV32I coverage: %d/%d result checks PASS, "
                  "37 distinct instructions executed %s",
             pass, ISA_CHECKS, pass == ISA_CHECKS ? "[PASS]" : "[FAIL]");
    ESP_LOGI(TAG, "");
}
